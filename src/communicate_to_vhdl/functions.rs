...
pub fn generate_vhd_communicator(
    ...
    declaration_code_output: String,
    construction_code: String,
    deconstruction_code: String
) -> Result<(),std::io::Error>{
...
Generic(
    ...
    RECIVING_SIZE: integer := {number_of_bit_in};

    -- the ammount of bit to recive
    SENDING_SIZE: integer := {number_of_bit_out};
...
architecture Behavioral of Comunicator is
    ...
    -- inputs
    {declaration_code_input}

    --output
    {declaration_code_output}
...
");
    ...
    let mut file = File::create("Comunicator.vhd")?;


    file.write_all(s.into_bytes().as_slice())?;
...
fn test(){
    generate_vhd_communicator(
        100,200,String::new(),String::new(),String::new(),String::new(),
    ).unwrap();
}